...
local function open_window()
	...
	local current_buff_name = (vim.fn.split(vim.api.nvim_buf_get_name(0), "/"))[#vim.fn.split(
	...
	for buffer in string.gmatch(ls, "([^\r\n]*)") do
		if string.match(buffer, "%d+") then
			if string.match(buffer, '%d+.-(a).-".-"') == "a" and string.match(buffer, '"(.-)"') ~= "[No Name]" then
				...
				table.insert(result, index .. " | " .. buffer)
			else
				buffer = string.match(buffer, '"(.-)"')
				table.insert(result, index .. " | " .. buffer)
			end
...
local function set_mappings()
	...
	local start_buffer = 1
	local end_buffer = 10

	for i = start_buffer, end_buffer do
